...
def evaluate(config):
    ...
    def sample_fn(_n_samples, category=-1, accelerator=None):
        ...
        _z = sampler.euler_sampler(nnet,_z_init, kwargs['y'], num_steps=50, cfg_scale=config.cfg/100., guidance_high=config.guidance).float()
...
flags.DEFINE_integer("cfg", -1, 'cfg / 100')
flags.DEFINE_float("cfg_scale_pow", -1, 'cfg decay')
flags.DEFINE_float("guidance", 0.7, 'guidance')
...
def main(argv):
    ...
    config.guidance = FLAGS.guidance
