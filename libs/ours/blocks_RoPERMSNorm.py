...
class OutBlock(nn.Module):
    def __init__(self, dim, channels, final_kernel_size):
        super().__init__()
        self.output1 = nn.Sequential(
            LayerNorm(dim),
            PixelShuffleUpsample(dim, dim),
            LayerNorm(dim),
            ChannelFirst(),
            nn.Conv2d(dim, channels, kernel_size = final_kernel_size, padding = final_kernel_size // 2),
        )
    ...
    def forward(self, x):
        return torch.cat([self.output1(x), self.output2(x), self.output3(x)], dim=1)
...
class Stage(nn.Module):
    def __init__(self, channels, config, lowres_dim = None, lowres_height = None):
        ...
        # elif config.block_type == "DCNv4_ConvNeXtBlock":
        #     self.encoder_cls = DCNv4_ConvNeXtBlock
        else:
            raise Exception()
        ...
        if config.pe_type == 'Axial_RoPE' and config.block_type == 'TransformerBlock':
            ...
            self.input_layer = nn.Sequential(
                EvenDownInterpolate(config.image_input_ratio),
                nn.Conv2d(channels*2 if config.input_feature_ratio==2 else channels, config.dim, kernel_size = config.input_feature_ratio, stride = config.input_feature_ratio),
                ChannelLast(),
        ...
        if config.block_type == 'TransformerBlock':
            self.output_layer = self.output_layer = nn.Sequential(
                LayerNorm(config.dim),
                ChannelFirst(),
                nn.Conv2d(config.dim, channels*3, kernel_size = config.final_kernel_size, padding = config.final_kernel_size // 2),
            )#OutBlock(config.dim, channels, config.final_kernel_size)
        else:
...
class FreqFlow(nn.Module):
    ...
    def _forward(self, images, log_snr, condition_classes = None, condition_text_embeds = None, condition_text_masks = None, condition_drop_prob = None):
        ...
        for idx, stage in enumerate(self.stages):
            if idx==0:
                low_x, high_x = Fourier_filter(images, D=8)
                output, lowres_skips = stage(torch.cat([low_x, high_x], dim=1), lowres_skips = lowres_skips, condition_classes = condition_classes, condition_embeds = condition_embeds, condition_masks = condition_masks)
