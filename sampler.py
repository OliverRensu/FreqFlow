...
def euler_sampler(
    ...
    steps = list(range(0, 1001, 20))[::-1]
    scale_steps=[]
    for i in steps:
        scale_step = (1-torch.cos(((1-torch.tensor([i])/1000)**2.0)*math.pi))*1/2
        scale_steps.append(scale_step.item())
    #t_steps = torch.tensor(scale_steps)#
    t_steps = torch.linspace(1, 0, num_steps+1, dtype=torch.float64)
    timestep_shift= 0.3
    numerator = timestep_shift * t_steps
    denominator = 1 + (timestep_shift - 1) * t_steps
    t_steps =  numerator / denominator
    ...
    with torch.cuda.amp.autocast():
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
            ...
            d_cur = model(
                model_input.to(dtype=_dtype), log_snr=time_input.to(dtype=_dtype)* 8 - 4, **kwargs
                )[1].to(torch.float64)
